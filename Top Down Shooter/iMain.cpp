...
void time_animation()
{
	...
	iSetColor(0, 0, 0);
	iShowImage(10, 686, 124, 24, time_UI);
	p.clock_current = (clock() - p.clock_start) / 1000;

	int_to_string(p.clock_current, current_time);

	iText(142 , 687, current_time, GLUT_BITMAP_TIMES_ROMAN_24);
...
void show_highscore()
{
	...
	struct Highscore *h;
    h = (struct Highscore *)malloc((in) * sizeof(struct Highscore));


	for(int i = 0; i < in; i++)
	{
		fscanf(fp2, "%d", &h[i].score);
		fscanf(fp1, "%s", h[i].name);
	}
	...
	{
		char s[100];
		iText(620, 500 - y_gap, h[i].name, GLUT_BITMAP_TIMES_ROMAN_24);

		int_to_string(h[i].score, s);

		iText(1020, 500 - y_gap, s, GLUT_BITMAP_TIMES_ROMAN_24);
...
void MenuLoad()
{
	...
	time_UI = iLoadImage("menu\\time.png");
